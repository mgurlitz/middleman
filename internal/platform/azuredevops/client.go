...
func WithCloneURLForTesting(cloneURL string) ClientOption {
	return func(opts *clientOptions) {
		opts.cloneURLOverride = cloneURL
	}
}
...
func (c *Client) GetRepository(ctx context.Context, ref platform.RepoRef) (platform.Repository, error) {
	...
	normalized := NormalizeRepository(c.host, scope.Org, scope.Project, repo)
	if c.cloneURLOverride != "" {
		normalized.CloneURL = c.cloneURLOverride
		normalized.Ref.CloneURL = c.cloneURLOverride
	}
...
func (c *Client) ListOpenMergeRequests(
	...
	for _, pr := range response.Value {
		normalized := NormalizePullRequest(c.host, scope, ref, pr)
		if c.cloneURLOverride != "" {
			normalized.Repo.CloneURL = c.cloneURLOverride
		}
