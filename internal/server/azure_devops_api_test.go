...
func TestAPIAzureDevOpsReadOnlySyncPersistsThroughServer(t *testing.T) {
	...
	provider, err := azuredevops.NewClient(
		...
		azuredevops.WithCloneURLForTesting(remote),
	)
	require.NoError(err)
	...
	srv := New(database, syncer, nil, "/", nil, ServerOptions{
		Clones: clones, WorktreeDir: t.TempDir(),
	})
	...
	repoRow, err := database.GetRepoByIdentity(ctx, azureDevOpsDBRepoIdentity(repo))
	require.NoError(err)
	...
	pullFilter := url.QueryEscape("azure_devops|dev.azure.com/AcmeOrg/Payments/Service")
	rawPulls := doJSON(t, srv, http.MethodGet, "/api/v1/pulls?repo="+pullFilter, nil)
	...
	rawWorkspace := doJSON(t, srv, http.MethodPost, "/api/v1/workspaces", map[string]any{
		"provider":      string(platform.KindAzureDevOps),
...
func TestAPIAzureDevOpsMergedEventsAppearInDetailAndActivityAfterClosure(t *testing.T) {
	...
	require.NoError(syncer.SyncMROnProvider(
		ctx, platform.KindAzureDevOps, platform.DefaultAzureDevOpsHost,
		repo.Owner, repo.Name, 17,
	))
	merged = true
	syncer.RunOnce(ctx)
	...
	require.NoError(json.NewDecoder(rawDetail.Body).Decode(&detail))
	require.NotNil(detail.MergeRequest)
	assert.Equal(db.MergeRequestStateMerged, detail.MergeRequest.State)
	assert.Equal(mergedAt, detail.MergeRequest.LastActivityAt)
	require.Len(detail.Events, 2)
	assert.Equal("merged", detail.Events[0].EventType)
	assert.Equal("merged this", detail.Events[0].Summary)
	assert.Equal("Merge Bot", detail.Events[0].Author)
	assert.Equal("issue_comment", detail.Events[1].EventType)
	assert.Equal("Looks good from Azure DevOps", detail.Events[1].Body)
	assert.Zero(detail.MergeRequest.CommentCount)
	...
	assert.Empty(activity.Items[0].BodyPreview)
}
...
func TestAPIAzureDevOpsSyncHandlesSpacesInRepoIdentity(t *testing.T) {
	...
	pullFilter := url.QueryEscape("azure_devops|dev.azure.com/Acme Org/Payments Team/Space Repo")
	rawPulls := doJSON(t, srv, http.MethodGet, "/api/v1/pulls?repo="+pullFilter, nil)
	require.Equal(http.StatusOK, rawPulls.Code, rawPulls.Body.String())
	var pulls []pullapi.MergeRequestResponse
	require.NoError(json.NewDecoder(rawPulls.Body).Decode(&pulls))
...
func azureDevOpsDBRepoIdentity(repo ghclient.RepoRef) db.RepoIdentity {
