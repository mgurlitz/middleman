...
func TestAPIAzureDevOpsReadOnlySyncPersistsThroughServer(t *testing.T) {
	...
	assert.Equal("approval", detail.Events[3].EventType)
	assert.Equal("approved this pull request", detail.Events[3].Summary)
	assert.Equal("Grace Hopper", detail.Events[3].Author)
	assert.Equal(1, detail.MergeRequest.CommentCount)
	assert.Equal(now.Add(7*time.Minute), detail.MergeRequest.LastActivityAt)
	...
	repoFilter := url.QueryEscape("dev.azure.com/AcmeOrg/Payments/Service")
